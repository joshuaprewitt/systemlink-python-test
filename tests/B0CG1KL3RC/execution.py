...
def _build_open_circuit_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="V",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="ocv_voltage", value=str(measured))],
...
def _build_under_load_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        high_limit=get_spec(specs, "voltage_high_limit"),
        units="V",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C"), NamedValue(name="load_current", value=f"{load_current} A")],
        outputs=[NamedValue(name="loaded_voltage", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_internal_resistance_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="mΩ",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="internal_resistance", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_capacity_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="mAh",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C"), NamedValue(name="charge_rate", value="1.0 A")],
        outputs=[NamedValue(name="measured_capacity", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_charge_voltage_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="V",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="charge_voltage", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_discharge_cutoff_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="V",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="cutoff_voltage", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_temperature_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="°C",
        inputs=[NamedValue(name="ambient_temp_c", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="cell_surface_temp", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
