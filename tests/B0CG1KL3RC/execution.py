...
def _build_open_circuit_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="V",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="OCV Voltage", value=str(measured))],
...
def _build_under_load_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        high_limit=get_spec(specs, "voltage_high_limit"),
        units="V",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C"), NamedValue(name="Load Current", value=f"{load_current} A")],
        outputs=[NamedValue(name="Loaded Voltage", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_internal_resistance_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="mΩ",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="Internal Resistance", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_capacity_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="mAh",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C"), NamedValue(name="Charge Rate", value="1.0 A")],
        outputs=[NamedValue(name="Measured Capacity", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_charge_voltage_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="V",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="Charge Voltage", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_discharge_cutoff_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="V",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="Cutoff Voltage", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_weight_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="g",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="Weight", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
...
def _build_temperature_step(result_id: str, ctx: TestContext, specs: dict[str, str], temp_c: float) -> CreateStepRequest:
    ...
    return build_step(
        ...
        units="°C",
        inputs=[NamedValue(name="Ambient Temp (°C)", value=f"{temp_c} °C")],
        outputs=[NamedValue(name="Cell Surface Temp", value=str(measured))],
        part_number=ctx.part_number,
        duration=duration,
        started_at=started_at,
    )
