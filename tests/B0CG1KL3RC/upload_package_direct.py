...
nipkg_files = sorted(dist_dir.glob('18650-battery-test_*.nipkg'))
if not nipkg_files:
    raise FileNotFoundError(f"No .nipkg found in {dist_dir}")
package_path = nipkg_files[-1]
...
parts.append(f'Content-Disposition: form-data; name="package"; filename="{package_path.name}"\r\n'.encode())
